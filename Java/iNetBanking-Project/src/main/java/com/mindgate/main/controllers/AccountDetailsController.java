...
public class AccountDetailsController {
	...
	public ResponseEntity<?> getById(@PathVariable("userId") int userId){
		try {
			ResponseEntity<List<AccountDetails> > responseEntity = new ResponseEntity<List<AccountDetails> >(accountDetailsServiceInterface.getById(userId), HttpStatusCode.valueOf(200));
